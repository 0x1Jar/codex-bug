...
- Focused skill references:
  - `Refrence/skills/plugins/firebase-apk-scanner/skills/firebase-apk-scanner/SKILL.md` when live traffic or extracted config points to Firebase
...
## Practical Android Follow-Ups

For Android dynamic analysis, use the secondary references like this:

- `AllDrozer-Commands.md` for exported component, provider, receiver, and intent-driven runtime testing
- `General-Commands.md` for emulator bootstrapping, `adb`, Frida server setup, and baseline Objection usage
- `Objection-All-Commands.md` when you need deeper runtime hooks, SSL pinning bypass variants, file access, or memory inspection
- `Pidcat-For-Logging.md` when log visibility matters more than raw `adb logcat`
- `frida-ssl-certificate-issue.md` and `Android16-CertInstall.md` when proxy trust or certificate installation blocks traffic interception
- `mobsf_docker_usage.md` when you want MobSF dynamic support against a connected emulator

If runtime inspection reveals Firebase project IDs, database URLs, storage buckets, or API keys, use the `firebase-apk-scanner` reference as a focused follow-up and then continue with `web2-recon` or `bug-bounty` if the lead becomes backend-heavy.
