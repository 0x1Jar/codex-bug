...
```bash
rg -n "MASVS-NETWORK|MASVS-STORAGE|MASVS-PLATFORM|MASVS-RESILIENCE" Refrence/maswe/weaknesses
rg -n "deep link|app link|webview|pinning|backup|allowBackup|ATS|entitlement" Refrence/mastg/Document
rg -n '"identifier": "MASVS-' Refrence/OWASP_MASVS.cdx.json
rg -n "drozer|objection|frida|deeplink|mobsf" Refrence/Learn-android-bug-bounty
find Refrence/skills -path '*/SKILL.md' | sort
```

## Secondary Reference Rules

- Use `Refrence/Learn-android-bug-bounty` as a practical Android operator cookbook, not as a replacement for MASVS, MASWE, or MASTG.
- Use `Refrence/skills` selectively:
  - Firebase-heavy APK or extracted config -> consult `Refrence/skills/plugins/firebase-apk-scanner/skills/firebase-apk-scanner/SKILL.md`
  - source tree available and deeper code scanning is useful -> consult `Refrence/skills/plugins/static-analysis/skills/semgrep/SKILL.md` and `Refrence/skills/plugins/static-analysis/skills/codeql/SKILL.md`
- Prefer the focused reference that best matches the current lead. Do not bulk-load unrelated skill references.
...
Use `commands/mobile.md` as the reference prompt doc for the overall workflow.
