...
- Focused skill references:
  - `Refrence/skills/plugins/firebase-apk-scanner/skills/firebase-apk-scanner/SKILL.md` for Firebase-backed Android APK leads
  - `Refrence/skills/plugins/static-analysis/skills/semgrep/SKILL.md` for source-tree scanning
  - `Refrence/skills/plugins/static-analysis/skills/codeql/SKILL.md` for deeper interprocedural source analysis
...
When the target is Android, these secondary references are fair game:

- `General-Commands.md` for quick emulator, `adb`, Frida, and Objection setup
- `deeplink_discovery_methods.md` when static review finds app links, custom schemes, or exported activities
- `mobsf_docker_usage.md` when you want fast triage or a second pass on APK metadata
- `firebase-apk-scanner` reference when the APK or strings reveal Firebase config, `google-services.json`, `firebaseio.com`, `appspot.com`, or `AIza`

When a source tree is available, use the static-analysis references as optional follow-up knowledge for Semgrep or CodeQL style scanning. Keep the main mobile output focused on mobile findings, not a generic SAST dump.
