...
elif [ "$(basename "$(dirname "$SCRIPT_DIR")")" = "modules" ]; then
    BASE_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
else
    BASE_DIR="$SCRIPT_DIR"
fi
...
if command -v assetfinder &>/dev/null; then
    ...
    # Ensure output file exists even if the command returns no results
    [ ! -f "$RECON_DIR/subdomains/assetfinder.txt" ] && touch "$RECON_DIR/subdomains/assetfinder.txt"
...
if [ ! -d "$WORDLIST_DIR" ] && [ -d "$BASE_DIR/tools/wordlists" ]; then
...
if command -v ffuf &>/dev/null && [ -s "$RECON_DIR/live/urls.txt" ]; then
    ...
    if [ -n "$WORDLIST" ]; then
        ...
        log_warn "No wordlist found — run: python3 hunt.py --setup-wordlists"
    fi
else
