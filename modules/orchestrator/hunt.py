...
def find_base_dir(script_dir):
    """Resolve repo root for modules/, root layout, and legacy tools/ layout."""
    ...
    current = normalized
    while True:
        if os.path.isdir(os.path.join(current, ".codex-plugin")):
            return current
...
TOOLS_DIR = LEGACY_TOOLS_DIR if os.path.isdir(LEGACY_TOOLS_DIR) else BASE_DIR
...
def resolve_tool_path(name):
    """Resolve tool path for modules-first, root wrappers, and legacy tools/."""
    tool_map = {
        "target_selector.py": os.path.join("modules", "orchestrator", "target_selector.py"),
        "recon_engine.sh": os.path.join("modules", "recon", "recon_engine.sh"),
        "vuln_scanner.sh": os.path.join("modules", "scanners", "vuln_scanner.sh"),
        "report_generator.py": os.path.join("modules", "reporting", "report_generator.py"),
        "cve_hunter.py": os.path.join("modules", "scanners", "cve_hunter.py"),
        "zero_day_fuzzer.py": os.path.join("modules", "scanners", "zero_day_fuzzer.py"),
    }

    candidates = [
        os.path.join(BASE_DIR, tool_map.get(name, name)),
        os.path.join(BASE_DIR, name),
        os.path.join(LEGACY_TOOLS_DIR, name),
    ]
...
def select_targets(top_n=10):
    ...
    script = resolve_tool_path("target_selector.py")
...
def run_recon(domain, quick=False):
    ...
    script = resolve_tool_path("recon_engine.sh")
...
def run_vuln_scan(domain, quick=False):
    ...
    script = resolve_tool_path("vuln_scanner.sh")
    quick_flag = "--quick" if quick else ""
...
def generate_reports(domain):
    ...
    script = resolve_tool_path("report_generator.py")
...
def run_cve_hunt(domain):
    ...
    script = resolve_tool_path("cve_hunter.py")
    recon_dir = os.path.join(RECON_DIR, domain)
...
def run_zero_day_fuzzer(domain, deep=False):
    ...
    script = resolve_tool_path("zero_day_fuzzer.py")
